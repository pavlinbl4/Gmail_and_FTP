"""
нужно найти файл  от ТАСС в папке Downloads 
критерий поиска начинается с "Павленко" и расширение xlsx
"""
...
main_report = "/Volumes/big4photo/Documents/TASS/Tass_total_report.xlsx"  # файл куда сохранятеся вся
...
def gen_х(sheet):  # функция определяет номер строки с которой начинается ввод данных в табицу
...
def gen_y(sheet):  # функция определяет номер строки с которой начинается ввод данных в табицу
    for row in sheet.iter_rows():
        for cell in row:
...
def get_prevue(file_to_work, options, report_date):
...
def add_information_to_main_file(file_to_work,
    ...
    x = gen_х(sheet)  # номер строки с которого начинается отчет по фото
    y = gen_y(sheet)
    photo_id = (sheet.cell(row=x, column=y)).value
...
# def get_report_date(file_name):  # получаю дату отчета в виде строки для новых отчетов
...
def get_report_date(file_name):  # для старых отчетов их xls кнвертированных файлов
    wb = load_workbook(f"{report_dir}/{file_name}")
    sheet = wb.active
    for row in sheet.iter_rows():
        for cell in row:
...
def move_and_rename(file_name, report_dir, destination):  # переименовываю и перемещаю файл отчета
    ...
    get_prevue(file_to_work, options, report_date)
